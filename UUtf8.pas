...
begin
  ...
  if Source <> nil then
    while Source[len] <> #0 do
      Inc(len);
  Result := UUtf8.UnicodeToUtf8(Dest, MaxBytes, Source, len);
end;
...
begin
  ...
  if Source <> nil then
    while Source[len] <> #0 do
      Inc(len);
  Result := UUtf8.Utf8ToUnicode(Dest, MaxChars, Source, len);
end;
...
begin
  ...
  L := UUtf8.UnicodeToUtf8(PChar(Temp), Length(Temp)+1, PWideChar(WS), Length(WS));
  if L > 0 then
    SetLength(Temp, L-1)
  else
    Temp := '';
...
begin
  ...
  L := UUtf8.Utf8ToUnicode(PWideChar(Temp), Length(Temp)+1, PChar(S), Length(S));
  if L > 0 then
    SetLength(Temp, L-1)
  else
    Temp := '';
